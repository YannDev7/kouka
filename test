...
# les bons
echo -n "bons "
for f in tests/typing/good/*.koka tests/exec/*.koka tests/exec-fail/*.koka; do
    echo -n ".";
    max=`expr $max + 1`;
    compile --type-only $f;
    case $? in
...
for f in tests/exec/*.koka; do
	...
	echo
	echo "ECHEC de l'interpr̩tation sur $f"
    fi
done
echo
...
for f in tests/exec-fail/*.koka; do
	...
	echo
	echo "ECHEC : le code $f devrait ̩chouer"
    else
	score_test=`expr $score_test + 1`;
    fi
done
...
case $option in
    ...
    * )
        ...
        echo "-i      : tester avec un interpr̬te"
